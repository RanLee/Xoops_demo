...
function ContentOrder($ObjOrder, $ObjectName, $FieldName){
    if ( isset($ObjOrder) || is_array($ObjOrder) ){
        $brand_handler =& xoops_getmodulehandler($ObjectName, 'catalog');
        foreach ($ObjOrder as $id=>$order){
    	    	$brand_obj = $brand_handler->get($id);
    	    	
        		$brand_obj->setVar($FieldName, $order);
        		if($brand_handler->insert($brand_obj)) unset($brand_obj);    	    	
        }     
        return true;
  	}
  	return false;
