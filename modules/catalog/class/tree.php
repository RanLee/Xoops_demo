...
if (!class_exists("catalogTree")) {
class catalogTree extends XoopsObjectTree {

    function catalogTree(&$objectArr, $rootId = null)
    {
    ...
    function _makeTreeItems($key, &$ret, $prefix_orig, $prefix_curr = '', $tags = null)
    {
        if ($key > 0) {
            if (count($tags)>0) foreach($tags as $tag) {
                ...
                $ret[$key]["cat_name"] = $this->_tree[$key]['obj']->getVar("cat_name");
            }
    ...
    function &makeSelBox($name, $prefix = '-', $selected = '', $EmptyOption = false, $key = 0)
    {
        ...
        $this->_makeSelBoxOptions("cat_name", $selected, $key, $ret, $prefix);
        $ret .= '</select>';
        return $ret;
    }
        ...
        if (isset($this->_tree[$key]['child'])) {
            foreach ($this->_tree[$key]['child'] as $childkey) {
                ...
                if (count($tags) > 0) {
                    ...
                    $ret['child'][$childkey]["cat_name"] = $this->_tree[$childkey]['obj']->getVar("cat_name");
                }
                endif;
